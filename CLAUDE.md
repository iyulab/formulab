...
`formulab` is a zero-dependency, tree-shakeable TypeScript library of 204 industrial/manufacturing calculation functions (plus 8 type guards) spread across 15 domains (quality, metal, chemical, electronics, construction, automotive, logistics, energy, safety, food, utility, battery, environmental, machining, ie). It ships as ESM only. Each function is a pure input→output calculation, often verified against a published engineering standard. Function/test counts are verified against the codebase by `pnpm check:function-counts` — see `README.md`'s Verification Status table for the counting convention.
...
3. **Error policy (see `ERRORS.md`):** invalid inputs throw `RangeError` with a descriptive message; functions must never return `NaN`/`Infinity` in output fields. When you change a function's validation behavior, update both its `@throws` JSDoc and the table in `ERRORS.md`. `pnpm check:nonfinite-outputs` (CI, after build) enforces the NaN/Infinity half by execution: it degenerates every numeric value in the literal-argument calls found in tests and fails on a non-finite output. A deliberate sentinel is allowed only by writing it into `ERRORS.md` as "`fn()` returns `field = Infinity`"; a new function needs at least one literal-argument test call or the check fails.
4. Verified functions carry **golden reference tests** asserting against published values from the cited standard — preserve and extend these; don't loosen a golden assertion to make a change pass.
   ...
   **A ratio, an inequality or the function's own output is not a golden.** In 2026-09 seven functions passed
   ...
   through the function — API 520 Examples 1 and 5, a published IEEE 1584-2002 worked example, the 18.5 ft
   fall-clearance figure. When a function cites a standard, find a worked example in it (or in a
   reference implementation that reproduces it) and pin the absolute number.
...
6. **Lookup-table transcription discipline.** Functions embedding a standard's lookup table (AP matrix, REBA tables, NIOSH multipliers, ISO 2859 sampling plans, …) are the library's highest-risk defect class: four independent transcription slips were found in 2026-07 (axis bands copied from a neighboring axis, irregular cells smoothed into a monotone pattern, invented extrapolated values, an off-by-one from an omitted column). When adding or touching such a table: (a) transcribe from the standard itself or a cell-complete reproduction — never reconstruct "plausible" values from the table's apparent pattern; (b) cite the exact table number in `@reference` and record the verification source; (c) pin **cell-level golden tests** including the table's irregular cells and band boundaries — a test asserting the implementation's own output is not a golden test; (d) test structural invariants where the standard states them (e.g. "S=1 → always L", merged rows, band coverage without gaps).
   ...
   Three rules the 2026-09 invariant sweep (`src/metal/tableInvariants.test.ts`, `src/quality/highRiskTableInvariants.test.ts`) learned by measurement:

   - **Assert the intermediate value, not only the final one.** A composition step can hide a broken cell: REBA's Table C saturates (C[1][1] and C[2][1] are both 1), so corrupting a Table A cell from 2 to 1 left `rebaScore` unchanged and the sweep passed. NIOSH is multiplicative, where a zero absorbs everything downstream. Assert the score or multiplier the table itself produces (`scoreA`, `fm`, `cm`) — the result types already expose them.
   - **Prove the detector before trusting a pass.** Every invariant here was verified by injecting the defect it claims to catch and watching it fail, then restoring. The flange sweep was validated by putting the pipe OD back into the raised-face column — the exact shape of the defect fixed in 0.47.0. Without that step, "all invariants pass" and "the invariants look at nothing" are indistinguishable.
   - **Know what monotone invariants cannot catch.** They catch ordering slips — a neighboring axis copied in, an off-by-one, an omitted column. They do not catch a wrong magnitude that stays inside the monotone envelope: changing NIOSH medium/f=8 from 0.35 to 0.38 kept every ordering intact and passed. That class is what (c)'s cell-level golden tests are for; the two are complements, not substitutes.
