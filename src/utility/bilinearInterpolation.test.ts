...
describe('bilinearInterpolation', () => {
  ...
  it('should throw RangeError for insufficient x points', () => {
    expect(() => bilinearInterpolation({
      x: [0], y: [0, 10], z: [[0, 10]], targetX: 0, targetY: 5,
    })).toThrow(RangeError);
  });

  it('should throw RangeError for mismatched z dimensions', () => {
    expect(() => bilinearInterpolation({
      x: [0, 10], y: [0, 10], z: [[0, 10]], targetX: 5, targetY: 5,
    })).toThrow(RangeError);
  });

  it('should throw RangeError for mismatched row length in z', () => {
    expect(() => bilinearInterpolation({
