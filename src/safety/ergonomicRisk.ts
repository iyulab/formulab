...
// Values per the published REBA worksheet (Hignett & McAtamney 2000)
const TABLE_A: number[][] = [
  //   Legs: 1  2  3  4
  /* Trunk=1, Neck=1 */ [1, 2, 3, 4],
  /* Trunk=1, Neck=2 */ [1, 2, 3, 4],
  /* Trunk=1, Neck=3 */ [3, 3, 5, 6],
  /* Trunk=2, Neck=1 */ [2, 3, 4, 5],
  /* Trunk=2, Neck=2 */ [3, 4, 5, 6],
  /* Trunk=2, Neck=3 */ [4, 5, 6, 7],
  /* Trunk=3, Neck=1 */ [2, 4, 5, 6],
  /* Trunk=3, Neck=2 */ [4, 5, 6, 7],
  /* Trunk=3, Neck=3 */ [5, 6, 7, 8],
  /* Trunk=4, Neck=1 */ [3, 5, 6, 7],
  /* Trunk=4, Neck=2 */ [5, 6, 7, 8],
  /* Trunk=4, Neck=3 */ [6, 7, 8, 9],
  /* Trunk=5, Neck=1 */ [4, 6, 7, 8],
  /* Trunk=5, Neck=2 */ [6, 7, 8, 9],
  /* Trunk=5, Neck=3 */ [7, 8, 9, 9],
];
...
// REBA Step 11 coupling scores
const COUPLING_SCORES: Record<NonNullable<RebaInput['coupling']>, number> = {
  good: 0,
  fair: 1,
  poor: 2,
  unacceptable: 3,
 ...
 *   Tables A/B/C and step adjustments follow the published REBA Employee Assessment
 *   Worksheet (Hedge) derived from that paper.
 ...
 *   `coupling` (Step 11: good/fair/poor/unacceptable → +0..+3) defaults to 'good'.
 */
export function ergonomicRisk(input: RebaInput): RebaResult {
  ...
  // --- Trunk Score ---
  // Flexion: 0°=1, 0-20°=2, 20-60°=3, >60°=4. Extension: 0-20°=2, >20°=3 (caps at 3).
  const absTA = Math.abs(input.trunkAngle);
  let trunkScore: number;
  if (absTA === 0) trunkScore = 1;
  else if (absTA <= 20) trunkScore = 2;
  else if (input.trunkAngle < 0) trunkScore = 3; // extension beyond 20°
  ...
  // --- Upper Arm Score ---
  // Flexion: ±20°=1, 20-45°=2, 45-90°=3, >90°=4. Extension beyond 20° = 2 (caps at 2).
  const absUAA = Math.abs(input.upperArmAngle);
  let upperArmScore: number;
  if (absUAA <= 20) upperArmScore = 1;
  else if (input.upperArmAngle < 0) upperArmScore = 2; // extension beyond 20°
  ...
  // Coupling score: good=0, fair=1, poor=2, unacceptable=3 (default 'good')
  const couplingScore = COUPLING_SCORES[input.coupling ?? 'good'];
