...
describe('roi', () => {
  ...
  it('should throw RangeError for zero investment', () => {
    expect(() => roi({ investment: 0, annualReturn: 1000, years: 5 })).toThrow(RangeError);
  });

  it('should throw RangeError for negative years', () => {
    expect(() => roi({ investment: 1000, annualReturn: 100, years: -1 })).toThrow(RangeError);
  });

  it('should throw RangeError for negative return', () => {
    expect(() => roi({ investment: 1000, annualReturn: -100, years: 5 })).toThrow(RangeError);
  });
});
